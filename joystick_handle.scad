...
module joystick_handle()
{
	...
	difference()
	{
		...
		translate([0,0,-zff])
			cylinder(h = (grip_height + grip_rest_thickness+ 2 * zff)-peg_height+peg_offset_z, r = (shaft_dia/2)-1);
...
module cms_stem()
{
	hymen_thickness = 0.3; // Should be your layer height.
	difference()
	{
		...
		translate([0,0,-zff]) cylinder(h = grip_cms_stub_length-4_way_hat_base_z-(hymen_thickness-0.1), r = 4_way_hat_base_dia/2);
	}
